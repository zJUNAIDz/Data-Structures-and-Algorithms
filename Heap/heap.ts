...
export class MaxHeap {
  ...
  private heapifyDown(idx: number): void {
    let largest = idx; // Assume the current index is the smallest
    const left = this.leftChild(idx); // Get the left child index
    const right = this.rightChild(idx); // Get the right child index

    // Check if the left child is larger than the current smallest
    if (left < this.length && this.data[left] > this.data[largest]) {
      largest = left;
    }
    // Check if the right child is larger than the current smallest
    if (right < this.length && this.data[right] > this.data[largest]) {
      largest = right;
    }
    if (largest === idx) return; // If the current index is already the smallest, stop

    // Swap the current element with the smallest child
    [this.data[idx], this.data[largest]] = [this.data[largest], this.data[idx]];
    this.heapifyDown(largest); // Recursively heapify down the affected subtree
  }
...
console.log(maxHeap.delete()); // 5
console.log(maxHeap.delete()); // 3
console.log(maxHeap.delete()); // 1
console.log(maxHeap.delete()); // -1
...
console.log(heap2.delete()); // 2
heap2.insert(1);
console.log(heap2.delete()); // 1
console.log(heap2.delete()); // 3
